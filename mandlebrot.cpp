...
int main ()
{
    ...
    // This is how many results we should have (because a rectangular or square image is x * y pixels)
    int values = height * width;

    int color = 255;

    // Create the PPM image with a file name and  to the output file
    // For documentation on how to set this up, look here: http://netpbm.sourceforge.net/doc/ppm.html
    std::ofstream fout("mandlebrot_image.ppm");
    fout << "P3\n" << width << ' ' << height << ' ' << color << '\n'; // This could have been done on multiple lines but I decided to do it in one
